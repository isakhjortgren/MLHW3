...
function res = grad(model, data, wd_coefficient)
  ...
  %% TODO - Write code here ---------------
    ...
    WD_to_hid = model.input_to_hid / norm(model.input_to_hid, 'fro');
    WD_to_out = model.hid_to_class / norm(model.hid_to_class, 'fro');
    dEdzk = 0;
    Error_derivative_out = dEdzk*hid_output';
    
    dEdzj = 
    Error_derivative_in = dEdzj*data.inputs';
